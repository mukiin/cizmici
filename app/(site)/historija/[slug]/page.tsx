...
import { getHistorija, getHistorijaAll, historijaNeighbors } from "@/lib/data";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const item = await getHistorija(slug);
...
export default async function HistorijaClanakPage({ params }: Props) {
  ...
  const { prev, next } = await historijaNeighbors(item.slug);

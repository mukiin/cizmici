...
export async function generateStaticParams() {
  const [putevi, infraObjekti] = await Promise.all([getPutevi(), getInfraObjekti()]);
  return [
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const [road, obj] = await Promise.all([getPut(slug), getInfraObjekt(slug)]);
  const item = road ?? obj;
...
export default async function InfraDetaljPage({ params }: Props) {
  const { slug } = await params;
  const [road, obj, infrastruktura] = await Promise.all([
    getPut(slug),
    getInfraObjekt(slug),
    getInfrastruktura(),
  ]);
  ...
  const neighbors = road ? await putNeighbors(road.slug) : { prev: undefined, next: undefined };
